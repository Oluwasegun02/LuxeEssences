...
def get_file_path(request, filename):
    ...
    nowTime = datetime.now().strftime('%Y%n%d%H:%M:%S')
...
def profile_path(request, filename):
    ...
    nowTime = datetime.now().strftime('%Y%n%d%H:%M:%S')
    filename = "%s%s" % (nowTime, original_filename)
    return os.path.join("uploadpro/", filename)
...
class Category(models.Model):
    ...
    image = models.ImageField(upload_to=get_file_path,
                              ...
                              width_field=None,
                              max_length=None,
                              blank=True,  # Allow blank for initial profiles without images
                              null=True,)
    ...
    meta_keyworlds = models.CharField(max_length=150, null=False, blank=False)
